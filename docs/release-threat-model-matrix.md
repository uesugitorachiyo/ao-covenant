...
| GitHub attestation gap | Generate GitHub artifact attestations for release assets; document direct attestation requirements in the [release attestation coverage map](release-attestation-coverage.md); verify manifest and replacement policy attestations. | `gh attestation verify manifest.json`, `gh attestation verify release-replacement-policy.json`, release workflow attestation step status, and consumer smoke output when attestation is enabled. | Block broad release promotion until attestation evidence is present or a documented exception is approved. |
| Unauthorized asset replacement | Fail closed when release assets already exist; require `replace_existing_assets=true`, a replacement reason, and generated replacement policy before publishing replacement assets. | `release-replacement-preflight-report.json`, `release-replacement-policy.json`, replacement policy schema validation, and replacement notice text. | Use the [release rollback runbook](release-rollback.md), publish replacement context, and ensure consumers validate the replacement policy. |
| Dry-run publish confusion | Default manual workflow dispatch to `dry_run=true`; dry runs upload workflow artifacts only and skip GitHub release publishing, artifact attestations, and post-release smoke verification. | `release-dry-run-artifact-audit.json`, dry-run workflow artifact upload, no GitHub release mutation, and explicit `dry_run=true` metadata. | Treat dry-run artifacts as rehearsal evidence only; publish through an intentional non-dry-run dispatch or tag push. |
...
| Sensitive material exposure | Keep private keys, credentials, production evidence bundles, unreleased bundles, and local paths out of public assets and issues; use public fixtures and redacted reports. | Repository hygiene tests, redaction checks, public docs safety warnings, `release-dry-run-artifact-audit.json` private-key checks, and issue-template routing. | Remove public exposure where possible, rotate affected secrets, route privately through the security process, and publish only minimal public status. |
